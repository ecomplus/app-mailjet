...
class Auth {
  ...
  async getAppToken (app) {
    return rq.post({
      ...
      body: { '_id': app.authentication.id },
      json: true
    })
  }
  ...
  async setAppToken (app, xstoreid) {
    try {
      sql.update({ app_token: app.access_token }, { x_store_id: xstoreid, authentication_id: app.my_id }, ENTITY).then(r => {
